...
var rowBtns = function(id){
	...
	"<button id='"+id+"' onclick='deletebnt(this)' class='btn btn-circle btn-3d btn-mn btn-danger' data-toggle='tooltip' data-placement='right' title='Delete'>  <span class='fa fa-minus-circle'></span> </button>" +
...
function getAllStudents(){
	...
	if (lastAllResultsMap.size == 0) {
		$.get("http://localhost:3500/getAllStudentData",{},
			function(response){
				...
				response.forEach(function(item){
					...
					let tr = "<tr>" +
										"<td> <input type='checkbox' name='checkbox1' id='" + item.reg_num + "'  ></td>" +
										"<td>" + item.reg_num + "</td>" +
		...
		for (const entry of lastAllResultsMap.entries() ) {
				...
				let tr = "<tr>" +
											"<td> <input type='checkbox' name='checkbox1' id='" + entry[0]  + "'  ></td>" +
											"<td>" + entry[0] + "</td>" +
											"<td>" + entry[1][2] + "</td>" +
											"<td>" + entry[1][3] + "</td>" +
											"<td>" + entry[1][4]  + "</td>" +
